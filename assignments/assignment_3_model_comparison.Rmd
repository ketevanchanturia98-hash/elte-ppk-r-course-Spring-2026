...
library(DataExplorer) # The "cheat code" for automated EDA
library(lmtest)
library(car)
```
...
# create_report(df) # disabled, but comments below indicate the analysis.

# outlier 50 in column-pain
# "woman" label in column-sex
```
...
```{r, message=FALSE, results='hide'}
df = df %>%
  mutate(pain = if_else(pain > 10, 10, pain)) %>%
  mutate(sex = if_else(sex == "woman", "female", sex))

df$sex = as.factor(df$sex)

# df = df[-c(142),]

create_report(df) # re-run to see if we missed anything
```
...
```{r}
# Cook’s Distance: this measures how much all the fitted values in the model change when a specific observation is removed.
plot(m1, which = 4)

# df[c(81, 123, 142), ]

# Bonferroni outlier test. This identifies observations with the most extreme studentized residuals.
outlierTest(m1)
```
...
```{r}
# Q-Q plot: if the residuals are normal, the points will fall approximately along the diagonal dashed line.
plot(m1, which = 2)

# Shapiro-Wilk Test: The null hypothesis is that the residuals are normally distributed. If p < 0.05, reject the null and assume non-normality.
shapiro.test(residuals(m1)) # p-value = 0.3601
```

Check the linearity assumption.

```{r}
# The first diagnostic plot (which = 1) is the residuals vs fitted plot
plot(m1, which = 1)

# The Rainbow Test checks if a linear fit is appropriate for the central subset of the data compared to the whole dataset. If p < 0.05, the relationship is likely nonlinear.
raintest(m1)
```
...
```{r}
# The 3rd diagnostic plot is specifically for homoscedasticity
plot(m1, which = 3)

bptest(m1) # p-value = 0.04904
# thus the null hypothesis of homoskedasticity is rejected and heteroskedasticity assumed...
```
...
```{r}
m2 = lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum + cortisol_saliva, data = df)
```
...
```{r}
plot(m2, which = 2)
shapiro.test(residuals(m2)) # p-value = 0.5528
```
...
AIC(m2) # 546.1959, less than AIC(m1), so m2 is better fit than m1
```

Compare the two models.

```{r}
anova(m1, m2) # p = 2.412e-10 meaning m2 explains more variance
```
